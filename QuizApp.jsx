...
//     setIsLoading(true);
//     try {
//       // Updated fetch URL to use port 8000 (backend port)
...
//       // Expect the backend to return data.quiz as an array of question objects
...
//   // Evaluate quiz answers and compute score
//   const handleSubmit = () => {
...
//     if (percentage < 50) return "You might need to review the fundamentals and practice more problems.";
//     if (percentage < 80) return "Good job! Consider revising some topics to achieve mastery.";
//     return "Excellent performance! You have a strong understanding of the material.";
//   };

//   // ---- Render Screens ----

//   // If the quiz has been submitted, show the results screen.
//   if (quizSubmitted) {
//     return (
...
//           <p className="mb-4">{getFeedback()}</p>
...
//                 className={currentQuestion === idx ? "bg-gray-800 text-white" : ""}
...
//                 strokeWidth="5"
//                 strokeDasharray={`${(score / questions.length) * 283 || 0} 283`}
//                 strokeLinecap="round"
//                 stroke="currentColor"
//                 fill="transparent"
//                 r="45"
...
const QuizApp = () => {
  ...
  const [quizSubmitted, setQuizSubmitted] = useState(false);
  const [score, setScore] = useState(0);
  // Dynamic score updates as answers change
  ...
  // Helper to extract a "concept" from question text based on keywords.
  const extractConcept = (text) => {
    const lowerText = text.toLowerCase();
    if (lowerText.includes("stack")) return "Stack";
    if (lowerText.includes("queue")) return "Queue";
    if (lowerText.includes("bubble")) return "Sorting (Bubble Sort)";
    if (lowerText.includes("binary search")) return "Binary Search";
    if (lowerText.includes("dijkstra")) return "Graph Algorithms";
    if (lowerText.includes("hash")) return "Hash Tables";
    if (lowerText.includes("sort")) return "Sorting Algorithms";
    // Fallback concept
    return "General";
  };

  // Evaluate quiz answers and compute score on final submission.
  const handleSubmit = () => {
    let calculatedScore = 0;
    questions.forEach(question => {
      if (answers[question.id] === question.correctAnswer) {
  ...
  // Compute feedback for which concepts to review based on incorrect answers.
  const getConceptsToReview = () => {
    const conceptsSet = new Set();
    questions.forEach(question => {
      if (answers[question.id] !== question.correctAnswer) {
        const concept = extractConcept(question.question);
  ...
  // If the quiz has been submitted, show the results screen.
  if (quizSubmitted) {
    const conceptsToReview = getConceptsToReview();
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-400 to-teal-600 p-8 flex items-center justify-center">
        <Card className="max-w-2xl mx-auto">
          <h2 className="text-xl font-bold mb-4">Quiz Results</h2>
          ...
          {conceptsToReview.length > 0 && (
            <div className="mb-4">
              <h3 className="font-semibold">Concepts to Review:</h3>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-400 to-teal-600 p-8">
      ...
      {/* Score/Progress Card showing dynamic score */}
      <Card className="max-w-2xl mx-auto mt-6">
        <div className="flex items-center justify-center p-6">
          <div className="relative">
            <svg className="w-24 h-24">
